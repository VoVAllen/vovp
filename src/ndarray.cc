...
// #include <vovp/runtime/shared_mem.h>
// #include <vovp/zerocopy_serializer.h>
...
inline size_t GetDataSize(const DLTensor& arr) {
  size_t size = 1;
  for (vovp_index_t i = 0; i < arr.ndim; ++i) {
    size *= arr.shape[i];
  }
...
struct NDArray::Internal {
  ...
  static void DefaultDeleter(NDArray::Container* ptr) {
    using vovp::runtime::NDArray;
    if (ptr->manager_ctx != nullptr) {
      static_cast<NDArray::Container*>(ptr->manager_ctx)->DecRef();
    } else if (ptr->dl_tensor.data != nullptr) {
      vovp::runtime::DeviceAPI::Get(ptr->dl_tensor.ctx)->FreeDataSpace(
  ...
  // that are not allocated inside of Vovp.
...
bool NDArray::IsContiguous() const {
  ...
  // See https://github.com/dmlc/vovp/issues/2118 and PyTorch's compute_contiguous() implementation
  int64_t z = 1;
...
void NDArray::CopyFromTo(DLTensor* from,
  ...
  CHECK_EQ(from_size, to_size)
    << "VovpArrayCopyFromTo: The size must exactly match";
  ...
  // api manager.
  VOVPContext ctx = from->ctx.device_type != kDLCPU ? from->ctx : to->ctx;
...
//   }
  ...
  CHECK(strm->Read(&reserved))
      << "Invalid DLTensor file format";
  CHECK(header == kVOVPNDArrayMagic)
      << "Invalid DLTensor file format";
  DLContext ctx;
  int ndim;
  DLDataType dtype;
...
// int vovpArrayAlloc(const vovp_index_t* shape,
//                   int ndim,
//                   int dtype_code,
//                   int dtype_bits,
//                   int dtype_lanes,
...
// int VovpArrayFree(VovpArrayHandle handle) {
//   API_BEGIN();
//   reinterpret_cast<NDArray::Container*>(handle)->DecRef();
//   API_END();
// }

// int VovpArrayCopyFromTo(VovpArrayHandle from,
//                        VovpArrayHandle to,
//                        VovpStreamHandle stream) {
//   API_BEGIN();
...
// int VovpArrayFromDLPack(DLManagedTensor* from,
//                        VovpArrayHandle* out) {
//   API_BEGIN();
...
// int VovpArrayToDLPack(VovpArrayHandle from, DLManagedTensor** out,
//                      int alignment) {
//   API_BEGIN();
...
// void VovpDLManagedTensorCallDeleter(DLManagedTensor* dltensor) {
//   (*(dltensor->deleter))(dltensor);
// }

// int VovpArrayCopyFromBytes(VovpArrayHandle handle,
//                           void* data,
//                           size_t nbytes) {
//   API_BEGIN();
//   VovpContext cpu_ctx;
...
//       << "VovpArrayCopyFromBytes: size mismatch";
...
// int VovpArrayCopyToBytes(VovpArrayHandle handle,
//                         void* data,
//                         size_t nbytes) {
//   API_BEGIN();
//   VovpContext cpu_ctx;
...
//   size_t arr_size = GetDataSize(*handle);
//   CHECK_EQ(arr_size, nbytes)
//       << "VovpArrayCopyToBytes: size mismatch";
//   DeviceAPI::Get(handle->ctx)->CopyDataFromTo(
//       handle->data, static_cast<size_t>(handle->byte_offset),
//       data, 0,
