...
class WorkspacePool::Pool {
 public:
  ...
  // allocate from pool
  void* Alloc(VOVPContext ctx, DeviceAPI* device, size_t nbytes) {
  ...
  void Release(VOVPContext ctx, DeviceAPI* device) {
...
void* WorkspacePool::AllocWorkspace(VOVPContext ctx, size_t size) {
...
void WorkspacePool::FreeWorkspace(VOVPContext ctx, void* ptr) {
